...
author: "Mathilde Josserand and Dan Dediu"
date: '`r date()`'
output:
  html_document: 
    fig_caption: yes
...
# For development, use less iterations overall:
final_run = TRUE;
if( !final_run ) warning("Not running the full model fitting: make sure you set final_run = TRUE for the actual runs!!!\n");

# For Bayesian regression with brms, try to use multiple cores:
brms_ncores = max(detectCores(all.tests=TRUE, logical=FALSE), 4, na.rm=TRUE);
# For Baysian regression, the burn-in and iterations:
brms_iter = ifelse(final_run,20000,3000); brms_warmup=ifelse(final_run,5000,500); brms_thin=ifelse(final_run,5,1); 
# For Bayesian regression, the control params to pass to Stan:
brms_control = list(adapt_delta=ifelse(final_run,0.999,0.85),  # avoid the "divergent transitions after warmup" warning
                    max_treedepth=ifelse(final_run,15,10)); # avoid the "max_treedepth" warning
# For Bayesian regressions I use HDI's and ROPEs for a sort of "significance" testing
# see https://cran.r-project.org/web/packages/bayestestR/vignettes/region_of_practical_equivalence.html
# I use CI=95% (larger than the "standard" 89% but smaller than 100% and a ROPE of -0.01 to 0.01
brms_ci = 0.95; brms_rope = c(-0.01, 0.01);

# Parallel kfold using futures:
plan(multiprocess);

# cache expensive results:
if( !dir.exists("./cached_results") ) dir.create("./cached_results");
...
# Verbal interpretation of BF cf the table above:
BF_interpretation <- function(BF, m1="m1", m2="m2")
{
  if( BF > 100 )   return (paste0("extreme evidence for ",m1));
  if( BF > 30 )    return (paste0("very strong evidence for ",m1));
  if( BF > 10 )    return (paste0("strong evidence for ",m1));
  if( BF > 3 )     return (paste0("moderate evidence for ",m1));
  if( BF > 1 )     return (paste0("anecdotal evidence for ",m1));
  if( BF == 1 )    return (paste0("no evidence"));
  if( BF > 0.33 )  return (paste0("anecdotal evidence for ",m2));
  if( BF > 0.10 )  return (paste0("moderate evidence for ",m2));
  if( BF > 0.033 ) return (paste0("strong evidence for ",m2));
  if( BF > 0.010 ) return (paste0("very strong evidence for ",m2));
  return (paste0("extreme evidence for ",m2));
}
```
...
# read file final
bNA <- read.table("./InputFiles/Database_final4.csv", header=T, sep=",", quote='"', fill=TRUE) 
#bNA <- read.table("/home/mathilde/Documents/Colors/Results_Blue_Daltonism/InputFiles/Database_final4.csv", header=T, sep=",", quote='"', fill=TRUE) 



# rescale altitude, dist2 with log base2
bNA$alt_rescaled <- log2(bNA$altitude)
bNA$alt_rescaled[bNA$alt_rescaled==-Inf] <- 0

bNA$log_dist2water <- log2(bNA$dist2water)
bNA$log_dist2rivers <- log2(bNA$dist2rivers)
bNA$log_dist2ocean <- log2(bNA$dist2ocean)
bNA$log_dist2lakes <- log2(bNA$dist2lakes)

bNA$log_dist2water[bNA$log_dist2water==-Inf] <- 0
bNA$log_dist2rivers[bNA$log_dist2rivers==-Inf] <- 0
bNA$log_dist2ocean[bNA$log_dist2ocean==-Inf] <- 0
bNA$log_dist2lakes[bNA$log_dist2lakes==-Inf] <- 0


# rescale longitude and latitude
bNA$lat_rescaled <- cos(bNA$latitude*(pi/180))
bNA$lon_rescaled <- cos(bNA$longitude*(pi/180))

# rescale lat_fam and long_fam
bNA$lat_fam_rescaled <- cos(bNA$lat_fam*(pi/180))
bNA$lon_fam_rescaled <- cos(bNA$lon_fam*(pi/180))

# rescale UVR with zscore
bNA$UVB_z <- scale(bNA$UVB)

# rescale daltonism to be able to use a Beta distribution
bNA$daltonism_beta <- bNA$daltonism/100
bNA$daltonism_beta[bNA$daltonism_beta==0] <- 0.0001
```
...
<img src="./InputFiles/hypothesis.jpg" alt="Hypothesis" style="height:in;"/>
...
In order to investigate our hypotheses, we analyzed a set of `r length(unique(bNA$glottocode))` *populations* dispersed across all continents. 
...
ggplot() + theme_bw() +
  ...
  geom_point(data= bNA, aes(x = longitude, y = latitude, color=longitude)) +
...
ggplot() + theme_bw() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, aes(x = longitude, y = latitude, color=latitude)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
# macroarea
pander(table(bNA$macroarea));

ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y = latitude, color=macroarea)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
The number of color categories (variable `Nb_color_cat`) varies between `r min(bNA$Nb_color_cat, na.rm=TRUE)` and `r max(bNA$Nb_color_cat, na.rm=TRUE)`, whereas variable `Blue` can only have the values `yes` or `no`.


```{r summary no color categories, echo=FALSE, message=FALSE, warning=FALSE}
# Visualize data
pander(summary(bNA$Nb_color_cat))
```
...
ggplot(data =bNA)+
  theme_bw() +
  geom_bar(aes(x=as.factor(Nb_color_cat), fill=as.factor(Nb_color_cat)), alpha=0.5) + 
...
ggplot() + theme_bw() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) , fill = "grey") +
  geom_point(data= bNA, aes(x = longitude, y = latitude, fill=Nb_color_cat), shape = 21, color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot(data =bNA)+
  theme_bw() +
  geom_bar(aes(x=as.factor(Blue), fill=as.factor(Blue)), alpha=0.5) + 
  labs(x="Number of color categories", fill="")+
  guides(fill=FALSE)
```
...
ggplot() + theme_bw() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=Blue), color="red") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=daltonism), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=UVB), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=climate_PC1), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=climate_PC2), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=log(altitude)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + 
  theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=log(dist2lakes)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=log(dist2rivers)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=log(dist2ocean)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=log(dist2water)), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
```{r summary hg, echo=FALSE, message=FALSE, warning=FALSE}
# Hunter gatherer (yes/no)
pander(summary(bNA$hg))
```

```{r distribution hg, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Subsistence strategy.")}
ggplot(data =bNA)+
  theme_bw() +
  geom_bar(aes(x=as.factor(hg), fill=as.factor(hg)), alpha=0.5) + 
  labs(x="Hunting-gathering subsistence strategy?", fill="")+
  guides(fill=FALSE)
```

```{r map hg, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of subsistence strategy (hunting-gathering).")}
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=hg), color="red") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=log_popSize), color="black") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
language family was obtained from [@bentz_evolution_2018]; there are `r length(unique(bNA$family_code))` uniques language families, most language sbelonging to the Indo-European, Afro-Asiatic and Uralic families. 
...
# language family
pander(summary(bNA$family_code))
```
...
ggplot(data =bNA)+
  theme_bw() +
  geom_bar(aes(x=as.factor(family_new), fill=as.factor(family_new)), alpha=0.5) + 
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = longitude, y = latitude, fill=family_new), color="black") +
...
subdf <- bNA[!duplicated(bNA$supra_fam_name), ]


ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group), fill = "grey") +
  geom_point(data= bNA, shape=21, aes(x = lon_fam, y = lat_fam), fill="red") +
  geom_text_repel(data= subdf, aes(x = lon_fam, y = lat_fam, label=supra_fam_name),hjust=0, vjust=0, size=4) +
...
```{r summary genetic distance, include=FALSE}
...
summary(bNA$V1_add_2)
summary(bNA$V2_add_2)
```
...
Those points have *marks* attached to them, and we focus here on two such marks: a categorical mark (Blue) and a continous mark (daltonism). 
...
lon2 <- dbdalto$longitude2 # with other longitude system
...
# create a ppp object with Blue as marked data
pppgrue <- ppp(lon, lat, xrange, yrange, marks=  dbdalto$Blue) 
pppgrue_num <- ppp(lon, lat, xrange, yrange, marks= as.numeric(dbdalto$Blue))


# convert ppp format to sp format
longlat <- data.frame(lon= dbdalto$longitude2, lat=dbdalto$latitude)
WGScoor <-  longlat
...
```{r PPA PPP Blue marks, echo=FALSE, message=FALSE, warning=FALSE}
pppgrue
```
...
```{r PPA densities without Blue, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Density surfaces for languages without a specific term for Blue.")}
spp <- split(pppgrue)
plot(density(spp[1]))
```

```{r PPA densities with Blue, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Density surfaces for languages with a specific term for Blue.")}
plot(density(spp[2]))
```
...
```{r PPA pairs, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Pairwise densitiy surfaces for Blue 'yes' and 'no'.")}
...
# read file final
gen.dist.add <-  read.table("./InputFiles/distmat_gen_add.csv", header=T, row.names=1, sep=",", quote='"', fill=TRUE) 
gen.dist.ult <-  read.table("./InputFiles/distmat_gen_ult.csv", header=T,  row.names=1, sep=",", quote='"', fill=TRUE)
gen.dist.na  <-  read.table("./InputFiles/distmat_gen_na.csv", header=T,  row.names=1, sep=",", quote='"', fill=TRUE) 

# change format of gen.dist.na so that it looks like the other dataformat
upperTriangle(gen.dist.na) <- lowerTriangle(gen.dist.na, byrow=TRUE)
diag(gen.dist.na) <- 0
```
...
```{r look at noimput matrix, echo=FALSE, message=FALSE, warning=FALSE}
pander(head(gen.dist.na[,1:6]))
```
...
qlcdist <- read.table("./InputFiles/qlc_data_dist.csv", header=T, sep=",", quote='"', row.names=1, fill=TRUE) 
qlcdist$rownam <- rownames(qlcdist)

qlcglotto <- read.table("./InputFiles/qlc2glottocode.csv", header=F, sep=",", quote='"', fill=TRUE) 
...
if( !file.exists("./InputFiles/elevation.Rdata") )
{
  library(elevatr);
  elevation <- get_elev_raster(WGScoor, z = 1, src = "aws")
  save(elevation, file="./InputFiles/elevation.Rdata", compress="xz", compression_level=9)
} else
{
  load("./InputFiles/elevation.Rdata")
}
...
Moran.I(as.numeric(dbdalto$Blue), data.dists.inv.m2) # with second distance method
```
...
# ... for Blue
Moran.I(as.numeric(dbdalto$Blue), data.dists.bin.2000)
Moran.I(as.numeric(dbdalto$Blue), data.dists.bin.1000)
...
moran.test(as.numeric(dbdalto$Blue), gg2, randomisation=FALSE)
```
...
moran.mc(as.numeric(dbdalto$Blue), gg2, nsim=99)
```
...
# Moran's I for number of color categories
Moran.I(dbdalto$Nb_color_cat, data.dists.inv.m2) 
...
gen_neighbour3 <- data.frame(dist_to_knn(as.matrix(gen.dist.na), k=clust)) # compute neighbouring list
...
gen_neighbour3$glottocode <- rownames(gen_neighbour3)
gen_neighbour3$ID <- as.numeric(1:nrow(gen_neighbour3))

# add Blue and daltonism
dbdalto$Blue2 <- as.numeric(dbdalto$Blue)
dbdalto$Blue2[dbdalto$Blue2==2] <-0
gen_neighbour$Blue<- dbdalto$Blue2[match(gen_neighbour$glottocode, dbdalto$glottocode)]
gen_neighbour$daltonism<- dbdalto$daltonism[match(gen_neighbour$glottocode, dbdalto$glottocode)]
phylo_neighbour$Blue<- dbdalto$Blue2[match(phylo_neighbour$glottocode, dbdalto$glottocode)]
phylo_neighbour$daltonism<- dbdalto$daltonism[match(phylo_neighbour$glottocode, dbdalto$glottocode)]
dist_neighbour$Blue<- dbdalto$Blue2[match(dist_neighbour$glottocode, dbdalto$glottocode)]
dist_neighbour$daltonism<- dbdalto$daltonism[match(dist_neighbour$glottocode, dbdalto$glottocode)]
gen_neighbour2$Blue<- dbdalto$Blue2[match(gen_neighbour2$glottocode, dbdalto$glottocode)]
gen_neighbour2$daltonism<- dbdalto$daltonism[match(gen_neighbour2$glottocode, dbdalto$glottocode)]
gen_neighbour3$Blue<- dbdalto$Blue2[match(gen_neighbour3$glottocode, dbdalto$glottocode)]
gen_neighbour3$daltonism<- dbdalto$daltonism[match(gen_neighbour3$glottocode, dbdalto$glottocode)]
...
rownames(gen_neighbour3) <- gen_neighbour3$ID
gen_neighbour3$ID <- NULL
...
for (el in 1:nrow(gen_neighbour3)){
  neighb <- as.numeric(gen_neighbour3[el, 2:clust])
  total_na[el,1] <-  gen_neighbour3$daltonism[el]
  total_na[el,2:ncol(total_na)] <- gen_neighbour3$daltonism[neighb]
}
...
# gather Blue data of neighbours for each focus population
...
for (el in 1:nrow(gen_neighbour)){
  neighb <- as.numeric(gen_neighbour[el, 2:clust])
  total_ult[el,1] <-  gen_neighbour$Blue[el]
  total_ult[el,2:ncol(total_ult)] <- gen_neighbour$Blue[neighb]
}

for (el in 1:nrow(gen_neighbour2)){
  neighb <- as.numeric(gen_neighbour2[el, 2:clust])
  total_add[el,1] <-  gen_neighbour2$Blue[el]
  total_add[el,2:ncol(total_add)] <- gen_neighbour2$Blue[neighb]
}

for (el in 1:nrow(phylo_neighbour)){
  neighb <- as.numeric(phylo_neighbour[el, 2:clust])
  total_phylo[el,1] <-  phylo_neighbour$Blue[el]
  total_phylo[el,2:ncol(total_phylo)] <- phylo_neighbour$Blue[neighb]
}

for (el in 1:nrow(dist_neighbour)){
  neighb <- as.numeric(dist_neighbour[el, 2:clust])
  total_dist[el,1] <-  dist_neighbour$Blue[el]
  total_dist[el,2:ncol(total_dist)] <- dist_neighbour$Blue[neighb]
}

for (el in 1:nrow(gen_neighbour3)){
  neighb <- as.numeric(gen_neighbour3[el, 2:clust])
  total_na[el,1] <-  gen_neighbour3$Blue[el]
  total_na[el,2:ncol(total_na)] <- gen_neighbour3$Blue[neighb]
}

# ------------------

# gather Blue data of neighbours for each focus population, with a MEAN number of neighbours
...
When the number of nearest neighbours is < 8, the genetic distance with ultrametric imputation explained best the patterning of our data points, for both Blue and daltonism. 
...
We first investigated if the number of color categories may predict variable Blue. 
...
In our dataset, only `r round((1 - length(bNA[bNA$Stage==0,])/length(bNA$Stage))*100,2) ` % of langage follow Berlin and Kay’s stages. 
The distributions of the number of color categories and the presence of a word for Blue overlaps:

```{r exploratory blue, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("The distribution of the number of color categories for language with (blue) and without (red) a word for Blue.")}
# DISTRIBUTION Nb_color_cat
ggplot(bNA, aes(x=Nb_color_cat, fill=Blue)) + 
...
# getting a yes if Nb_color_cat >=6, and no if Nb_color_cat<6
yes_percent_6 <- summary(bNA[bNA$Nb_color_cat >= 6,]$Blue)["yes"] / sum(summary(bNA[bNA$Nb_color_cat>=6,]$Blue))
no_percent_6  <- summary(bNA[bNA$Nb_color_cat  < 6,]$Blue)["yes"] / sum(summary(bNA[bNA$Nb_color_cat<6,]$Blue))
```
...
<img src="./InputFiles/schema_explain_mediation.png" alt="Mediation path diagrams." style="height:in;"/>
...
{
  ## WITH ALL DATA, with normal regression: UVR --> Nb_color_cat --> Blue
  model_mediator <- bf(Nb_color_cat ~ UVB_z + (1 | macroarea) + (1 | family_code)) + gaussian()
  model_outcome <- bf(Blue ~ UVB_z + Nb_color_cat + (1 | macroarea) + (1 | family_code)) + bernoulli(link = "logit")
  
  blue_UV_Nbcol <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)
  
  ## WITH ALL DATA, with normal regression: lat --> UVB_z --> Blue
  model_mediator <- bf(UVB_z ~ lat_rescaled + (1 | macroarea) + (1 | family_code)) + gaussian()
  model_outcome <- bf(Blue ~ UVB_z + lat_rescaled + (1 | macroarea) + (1 | family_code)) + bernoulli(link = "logit")
  blue_lat_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)

  # ## WITH ALL DATA, with normal regression: lat² --> UVB_z --> Blue
  # model_mediator <- bf(UVB_z ~ I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code)) + gaussian()
  # model_outcome <- bf(Blue ~ UVB_z + I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code)) + bernoulli(link = "logit")
  # blue_lat2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
  #           cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
  #           control=brms_control)

  ## WITH ALL DATA, with normal regression: lat + lat² --> UVB_z --> Blue
  model_mediator <- bf(UVB_z ~ lat_rescaled + I(lat_rescaled^2)  + (1 | macroarea) + (1 | family_code)) + gaussian()
  model_outcome <- bf(Blue ~ UVB_z  + lat_rescaled +  I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code)) + bernoulli(link = "logit")
  blue_latlat2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)

  ## WITH ALL DATA, with normal regression: lat_fam --> UVB_z --> Blue
  model_mediator <- bf(UVB_z ~ lat_fam_rescaled + (1 | macroarea)) + gaussian()
  model_outcome <- bf(Blue ~ UVB_z + lat_fam_rescaled + (1 | macroarea)) + bernoulli(link = "logit")
  blue_latfam_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)

  # ## WITH ALL DATA, with normal regression: lat_fam² --> UVB_z --> Blue
  # model_mediator <- bf(UVB_z ~ I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code)) + gaussian()
  # model_outcome <- bf(Blue ~ UVB_z + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code)) + bernoulli(link = "logit")
  # blue_latfam2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
  #           cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
  #           control=brms_control)

  ## WITH ALL DATA, with normal regression: lat_fam + lat_fam² --> UVB_z --> Blue
  model_mediator <- bf(UVB_z ~ lat_fam_rescaled + I(lat_fam_rescaled^2) + (1 | macroarea)) + gaussian()
  model_outcome <- bf(Blue ~ UVB_z  + lat_fam_rescaled + I(lat_fam_rescaled^2) + (1 | macroarea)) + bernoulli(link = "logit")
  blue_latfamlatfam2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)
...
plot_answer_med <- function(x, outcome, mediator){
  if (outcome=="daltonismbeta" & mediator =="UVBz"){
    outcome1 <- "Daltonism"
    mediator1 <- "UVR"
  } else if (outcome=="Blue" & mediator =="UVBz"){
    outcome1 <- "Blue"
    mediator1 <- "UVR"
  } else if (mediator=="Nbcolorcat" & outcome=="Blue"){
...
```{r show mediation lat2 UV Blue, echo=FALSE, message=FALSE, warning=FALSE}
##### Latitude quadratic
# mediation(blue_lat2_UV)
# pp_check(blue_lat2_UV, resp = 'Blue') + ggtitle('Outcome (Blue)')
...
# 
# plot_answer_med(blue_latlat2_UV, "Blue", "UVBz")
...
```{r show mediation latfam2 UV Blue, echo=FALSE, message=FALSE, warning=FALSE}
##### Latitude quadratic
# mediation(blue_latfam2_UV)
# pp_check(blue_latfam2_UV, resp = 'Blue') + ggtitle('Outcome (Blue)')
...
# 
# plot_answer_med(blue_latfamlatfam2_UV, "Blue", "UVBz")
...
On the other hand, latitude (and latitude^2^) does have an effect on Blue, but this is fully mediated by UVR incidence (when having macroarea and family as random effects).
...
We model here the effects of UVR incidence on Blue using Bayesian mixed-effects models (implemented in package `brms`; @brms_2018). 
...
if( !file.exists("./cached_results/blue_uvb.RData") )
{
  ...
  model_uvb_rs <- brm(Blue ~ UVB_z + (1 + UVB_z | macroarea) + (1 + UVB_z | family_code), 
                      data=bNA, # very similar notation to lmer
  ...
  model_uvb_nors <- update(model_uvb_rs, . ~ . - (1 + UVB_z | macroarea) - (1 + UVB_z | family_code) + (1 | macroarea) + (1 | family_code), cores=brms_ncores);
  ...
  # Check if UVB_z matters:
  model_uvb_0 <- update(model_uvb, . ~ . - UVB_z, cores=brms_ncores);
  summary(model_uvb_nors);
  ...
  for (k in 1:30){
    ...
    data_split <- initial_split(bNA, prop=0.80, strata = "daltonism")
    data_train <- training(data_split)
    data_test <- testing(data_split)
  
    # fit model on training dataset
    fit_train <-  brm(Blue ~  UVB_z + (1 | macroarea) + (1 | family_code),
                      data=data_train,
                      family=bernoulli(link = "logit"),
                      cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
    ...
    ConfusionMatrix <- table(Pred, pull(data_test, Blue)) #`pull` results in a vector
...
UVR has a *strong negative* effect on Blue (Bayes factor `r sprintf("%.2f",bf_uvb_0$bf)`, `r BF_interpretation(bf_uvb_0$bf, "model with UVR incidence", "null model")`; `r brms_ci*100`% HDI for *&beta;*~UVR~ = [`r round(m_hdi$CI_low[m_hdi$Parameter == "b_UVB_z"],2)`, `r round(m_hdi$CI_high[m_hdi$Parameter == "b_UVB_z"],2)`]), explaining R^2^ = `r round(Rsquared[1]*100,2)`% of the variance, and correctly classifying `r round(accuracy_brms*100, 2)`% of the populations (recall = `r round(recall_brms*100, 2)`%, precision = `r round(precision_brms*100,2)`%).
...
if( !file.exists("./cached_results/blue_uvb_best.RData") )
{
  ...
  # adding lat_fam quadratic brought 1729
  # adding latitude quadratic brought 1205
  # adding number of color cat quadratic brought 1131 (bayes factor)
  # adding lat_fam brought 181
  # adding distance to lakes brought 143 (bayes factor)
  # adding UVB  brought 93 (bayes factor)
  # adding lon_fam quadratic brought 93 (bayes factor)
  # adding latitude brought 79 (bayes factor)
  # adding hg brought 14 (bayes factors)
  # adding longitude brought 7.66 (bayes factor)
  # adding lon_fam brought 5.47 (bayes factor)
  
  bestmodel <- brm(Blue ~ UVB_z + Nb_color_cat + log_dist2lakes + hg + lat_fam_rescaled + lon_fam_rescaled + lat_rescaled + lon_rescaled + I(lat_fam_rescaled^2) + I(lon_fam_rescaled^2) +  I(lat_rescaled^2) + I(Nb_color_cat^2) + (1 | macroarea) + (1 | family_code), 
                   data=bNA, # very similar notation to lmer
                   family=bernoulli(link = "logit"),
                   cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
mc_best <- glm(Blue ~ UVB_z + Nb_color_cat + log_dist2lakes + hg + lat_fam_rescaled + lon_fam_rescaled + lat_rescaled + lon_rescaled, data=bNA, family=binomial(link = "logit")); 

pander(vif(mc_best))
```

UVR and latitude have the highest VIF scores in our dataset, but, according to our mediation analysis, latitude has an effect on Blue only through UVR. 
...
if( !file.exists("./cached_results/blue_uvb_best_vif.RData") )
{
  ...
  model_nolat <- brm(Blue ~ UVB_z + (1 | macroarea) + (1 | family_code), 
                     data=bNA, # very similar notation to lmer
                     family=bernoulli(link = "logit"),
                     cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
  ...
  model_nouvb_quadr <- brm(Blue ~ I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code), 
                           data=bNA, # very similar notation to lmer
                           family=bernoulli(link = "logit"),
                           cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
  
  # model with only latitude linear (no uvb)
  model_nouvb_lin <- brm(Blue ~ lat_rescaled + (1 | macroarea) + (1 | family_code), 
                         data=bNA, # very similar notation to lmer
                         family=bernoulli(link = "logit"),
                         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
  
  # model with both latitude quadratic linear (no uvb)
  model_nouvb <- brm(Blue ~ I(lat_rescaled^2) + lat_rescaled + (1 | macroarea) + (1 | family_code), 
                     data=bNA, # very similar notation to lmer
                     family=bernoulli(link = "logit"),
                     cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
if( !file.exists("./cached_results/blue_uvb_remove_uvb.RData") )
{
  ...
  bestmodel_nouvb <- brm(Blue ~ Nb_color_cat + log_dist2lakes + hg + lat_fam_rescaled + lon_fam_rescaled + lat_rescaled + lon_rescaled + 
                           I(lat_fam_rescaled^2) + I(lon_fam_rescaled^2) +  I(lat_rescaled^2) + I(Nb_color_cat^2) + (1 | macroarea) + (1 | family_code), 
                         data=bNA, # very similar notation to lmer
                         family=bernoulli(link = "logit"),
                         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
if( !file.exists("./cached_results/blue_uvb_remove_lat.RData") )
{
  ...
  bestmodel_nolat <- brm(Blue ~ Nb_color_cat + log_dist2lakes + hg + lat_fam_rescaled + lon_fam_rescaled  + lon_rescaled + 
                           I(lat_fam_rescaled^2) + I(lon_fam_rescaled^2) + I(Nb_color_cat^2) + (1 | macroarea) + (1 | family_code), 
                         data=bNA, # very similar notation to lmer
                         family=bernoulli(link = "logit"),
                         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
Thus, we should keep, UVR and latitude (and latitude^2^) as predictors of Blue. 
...
{
  bestlightmodel <- brm(Blue ~ Nb_color_cat + dist2lakes + I(Nb_color_cat^2) + (1 | macroarea) + (1 | family_code), 
                        data=bNA, # very similar notation to lmer
                        family=bernoulli(link = "logit"),
                        cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control); 
...
{
  bestmodel2 <- brm(Blue ~ UVB_z + log_popSize + log_dist2lakes + hg + lat_fam_rescaled + lon_fam_rescaled + lat_rescaled + lon_rescaled + 
                      I(lat_fam_rescaled^2) + I(lon_fam_rescaled^2) +  I(lat_rescaled^2)  + (1 | macroarea) + (1 | family_code), 
                    data=bNA, # very similar notation to lmer
                    family=bernoulli(link = "logit"),
                    cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control); 
...
{
  bestmodel2_nouvb <- brm(Blue ~ log_popSize + log_dist2lakes + hg + lat_fam_rescaled + lon_fam_rescaled + lat_rescaled + lon_rescaled + 
                            I(lat_fam_rescaled^2) + I(lon_fam_rescaled^2) +  I(lat_rescaled^2)  + (1 | macroarea) + (1 | family_code), 
                          data=bNA, # very similar notation to lmer
                          family=bernoulli(link = "logit"),
                          cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control); 
...
{
  bestmodel2_kfold <- brm(Blue ~ UVB_z + log_popSize + log_dist2lakes + (1 | macroarea) + (1 | family_code), 
                          data=bNA, # very similar notation to lmer
                          family=bernoulli(link = "logit"),
                          cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control); 
...
{
  bestmodel2_kfold_nouvb <- brm(Blue ~ log_popSize + log_dist2lakes  + (1 | macroarea) + (1 | family_code), 
                                data=bNA, # very similar notation to lmer
                                family=bernoulli(link = "logit"),
                                cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control); 
...
```{r plot predictor blue pop size, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Population size vs having a word for Blue.")}
ggplot(bNA, aes(x=Blue, y=log_popSize, color=Blue)) + 
  theme_bw() +
...
```{r plot predictor blue uv, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("UVR incidence vs having a word for Blue.")}
ggplot(bNA, aes(x=Blue, y=UVB_z, color=Blue)) + 
  theme_bw() +
  geom_point(alpha=0.45, position=position_jitter(h=0.0,w=0.15)) + 
  geom_violin(alpha=0.3) +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
             ...
             legend.text=element_text(size =15)) +
  guides(color=FALSE) + 
  labs(y = "UV incidence (z scored)", x="Blue") ;
```

```{r plot predictor blue uv 2, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of UVR incidence on having a word for Blue.")}
# plot latitude 
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y =latitude, fill=UVB, shape=Blue), color="red") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "UVR incidence", shape="Blue") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
...
```{r plot predictor blue lat, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Latitude on having a word for Blue.")}
# violin plot latitude 
ggplot(bNA, aes(x=Blue, y=lat_rescaled, color=Blue)) + 
  theme_bw() +
  geom_point(alpha=0.45, position=position_jitter(h=0.0,w=0.15)) + 
  geom_violin(alpha=0.3) +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
...
```{r plot predictor blue dist2lakes, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of log(dist2lakes) on having a word for Blue.")}
# plot latitude 
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y =latitude, fill=log(dist2lakes), shape=Blue), color="red") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(fill = "log(dist2lakes)", shape="Blue") +
  scale_fill_gradient(low = "lightskyblue1", high = "navyblue") + 
  scale_shape_manual(values=c("no"=24, "yes"=21))
```

```{r plot predictor blue dist2lakes 2,echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("log(dist2lakes) on having a word for Blue.")}
ggplot(bNA, aes(x=Blue, y=log(dist2lakes), color=Blue)) + 
  theme_bw() +
  geom_point(alpha=0.45, position=position_jitter(h=0.0,w=0.15)) + 
  geom_violin(alpha=0.3) +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
...
```{r plot predictor blue latfam, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of latitude of language family origins on having a word for Blue.")}
# plot latitude of language family origins
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA, aes(x = longitude, y = latitude, color=lat_fam_rescaled, shape=Blue)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(color = "Latitude of language family origin", shape="Blue") 
```

```{r plot predictor blue lonfam, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of longitude of language family origins on having a word for Blue.")}
# plot longitude of language family origins
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y = latitude, color=lon_fam_rescaled, shape=Blue)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(color = "Longitude of language family origin", shape="Blue") 
```

```{r plot predictor blue latfam and lonfam, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Location of putative origins of language family on having a word for Blue.")}
# violin plot: latitude on the left, longitude on the right
plat <- ggplot(bNA, aes(x=Blue, y=lat_fam_rescaled, color=Blue)) + 
  theme_bw() +
  geom_point(alpha=0.45, position=position_jitter(h=0.0,w=0.15)) + 
  geom_violin(alpha=0.3) +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
...
plon <- ggplot(bNA, aes(x=Blue, y=lon_fam_rescaled, color=Blue)) + 
  theme_bw() +
  geom_point(alpha=0.45, position=position_jitter(h=0.0,w=0.15)) + 
  geom_violin(alpha=0.3) +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
...
```{r plot predictor blue hg, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Map of subsistence strategy on having a word for Blue.")}
# plot latitude 
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y =latitude, color=hg, shape=Blue)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(color = "Hunting-gathering?", shape="Blue") +
  scale_shape_manual(values=c("no"=24, "yes"=21)) + 
  scale_color_manual(values=c("no"="red", "yes"="blue"))

tmp <- bNA[,c("hg", "Blue")];
tmp$hg <- paste0("HG:",tmp$hg);
tmp$Blue <- paste0("Blue:",tmp$Blue);
pander(table(tmp))
```
...
if( !file.exists("./cached_results/blue_var_importance.RData") )
{
  ...
  list_variables <- c("UVB_z" , "daltonism" , "hg" , "lat_rescaled", "lon_rescaled" , "log_popSize" , "climate_PC1", "climate_PC2" , "alt_rescaled"  , "log_dist2water", "log_dist2ocean" , "log_dist2lakes", "log_dist2rivers" , "Nb_color_cat" , "lat_fam_rescaled", "lon_fam_rescaled")
  ...
  for (it in 1:n_iter_gini)
  {
    ...
    rf.db = randomForest(Blue ~  UVB_z + daltonism  + hg + lat_rescaled + lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2water + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, data = bNA, importance=TRUE)
    ...
    obj <- cforest(Blue ~  UVB_z + daltonism  + hg +  lat_rescaled +lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2water + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, data = bNA)
    
    # compute classification accuracy, precision and recall for cforest algorithm
    oobPredicted <- predict(obj,OOB=T)
    ConfusionMatrix <- table(bNA$Blue, oobPredicted)
...
ggplot(mapping = aes(x = reorder(namevar, mean_all), y = mean_all), 
       ...
       scale_x_discrete(labels=c("hg" = "Subsistence mode", "lon_fam_rescaled" = "Long of family origin", "lat_fam_rescaled" = "Lat of family origin", "log_dist2ocean" = "Dist to ocean","log_dist2rivers" = "Dist to rivers", "lon_rescaled" = "Longitude", "alt_rescaled" = "Altitude", "log_dist2water" = "Dist to water", "climate_PC1" = "Climate PC1", "climate_PC2" = "Climate PC2", "log_dist2lakes" = "Dist to lakes", "lat_rescaled" = "Latitude", "log_popSize" = "Population size", "UVB_z" = "UVR","daltonism" = "Daltonism", "Nb_color_cat" = "Nb of colour categ")) +
       guides(fill=FALSE)
```
...
if( !file.exists("./cached_results/blue_nonbcolors_var_importance.RData") )
{
  ...
  # choose variable to analyze
  list_variables <- c("UVB_z" , "daltonism"  , "hg" , "lat_rescaled", "lon_rescaled" , "log_popSize" , "climate_PC1", "climate_PC2" , "alt_rescaled"  , "log_dist2water", "log_dist2ocean" , "log_dist2lakes", "log_dist2rivers"  , "lat_fam_rescaled", "lon_fam_rescaled")
  ...
  for (it in 1:n_iter_gini){
    # compute random forest algorithm on all variable except multicollinear
    rf.db = randomForest(Blue ~  UVB_z + daltonism  + hg + lat_rescaled + lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled + log_dist2water  + log_dist2ocean + log_dist2lakes + log_dist2rivers  + lat_fam_rescaled + lon_fam_rescaled, data = bNA, importance=TRUE)
    ...
    # use an other algorithm to compute the accuracy-based variable importance
    obj <- cforest(Blue ~  UVB_z + daltonism  + hg + lat_rescaled + lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2water + log_dist2ocean + log_dist2lakes + log_dist2rivers  + lat_fam_rescaled + lon_fam_rescaled, data = bNA)
...
ggplot(mapping = aes(x = reorder(namevar, mean_all), y = mean_all), 
       ...
       theme(axis.text.x = element_text(angle = 60, hjust = 1, size =13, face = "bold"), 
             ...
             axis.title=element_text(size=16),
             legend.position="none") +
  scale_x_discrete(labels=c("hg" = "Subsistence mode", "lon_fam_rescaled" = "Long of family origin", "lat_fam_rescaled" = "Lat of family origin", "log_dist2ocean" = "Dist to ocean","log_dist2rivers" = "Dist to rivers", "lon_rescaled" = "Longitude", "alt_rescaled" = "Altitude", "log_dist2water" = "Dist to water", "climate_PC1" = "Climate PC1", "climate_PC2" = "Climate PC2", "log_dist2lakes" = "Dist to lakes", "lat_rescaled" = "Latitude", "log_popSize" = "Population size", "UVB_z" = "UVR","daltonism" = "Daltonism")) +
       guides(fill=FALSE)
```
...
# fit <- rpart(Blue ~ UVB + daltonism  + hg + longitude + latitude + log_popSize + climate_PC1 + climate_PC2 + altitude + dist2water + dist2ocean + dist2lakes + dist2rivers + Nb_color_cat, data = bNA, method = 'class', model=TRUE, control=control)
#rpart.plot(fit, extra = 106)
tree <- ctree (Blue ~  UVB_z + daltonism  + hg + lon_rescaled + lat_rescaled + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled + log_dist2water + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, 
       data = bNA, 
...
bNA_map <- bNA
# create range
bNA_map$PC2_range <- cut(bNA_map$climate_PC2, c(-Inf, nb_PC2, Inf), 
                                labels=c(paste("less than", nb_PC2), paste("more than", nb_PC2)))

bNA_map$UVB_range <- cut(bNA_map$UVB_z, c(-Inf, nb_UVB, Inf), 
                                labels=c(paste("less than", nb_UVB), paste("more than", nb_UVB)))

bNA_map$dlakes_range <- cut(bNA_map$log_dist2lakes, c(-Inf, nb_distlake, Inf), 
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(aes(x = bNA_map$longitude, y = bNA_map$latitude, color=bNA_map$UVB_range, shape=bNA_map$Blue)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(color="UVR (z-scored)", shape= "Blue")
```
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(aes(x = bNA_map$longitude, y = bNA_map$latitude, color=bNA_map$dlakes_range, shape=bNA_map$Blue)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
  labs(color="Log dist to lakes", shape= "Blue")
```

Thus, it seems that the distance to lakes is relevant only when there are exactly 6 color categories, in which case populations living more than about 25 km (= 2^4.7^) away from the closest lake do not have a specific word for Blue, whereas population living closer have a 75% chance to possess such a word. 
...
if( !file.exists("./cached_results/blue_mach_learning.RData") )
{
  ...
  for (k in 1:30){
    ...
    # split data in training and testing set
    data_split <- initial_split(bNA, prop=0.80, strata = "Blue")
    data_train <- training(data_split)
    data_test <- testing(data_split)
    ...
    rf.db = randomForest(Blue ~ UVB + daltonism  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, data = data_train)
    
    # create confusion matrix
    pred <- predict(rf.db, data_test, type='class')
    ConfusionMatrix <- table(data_test$Blue, pred)
    ...
    # fit model
    fit_train <- brm(Blue ~ UVB + daltonism  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled + (1 | macroarea) + (1 | family_code), 
    ...
    tuned_parameters <- tune.svm(Blue ~ UVB + daltonism  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, 
                                 data = data_train, 
                                 gamma = 10^(-5:-1), 
                                 cost =10^(-3:1))
  
    # fit SVM with tuned parameters on training data
    svm_model_after_tune <- svm(Blue ~ UVB + daltonism  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, 
                                data_train, 
                                kernel="radial", 
...
if( !file.exists("./cached_results/blue_mach_learning_simple.RData") )
{
  ...
  for (k in 1:30){
    ...
    # fit randomforest on training dat
    rf.db = randomForest(Blue ~  Nb_color_cat + log_dist2lakes + I(Nb_color_cat^2), data = data_train)
    ...
    # fit model
    fit_train <- brm(Blue ~ Nb_color_cat + log_dist2lakes + I(Nb_color_cat^2) + (1 | macroarea) + (1 | family_code), 
                        data=data_train, # very similar notation to lmer
                        family=bernoulli(link = "logit"),
                        cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
                        control=brms_control); 
    ...
    # find best parameters for SVM
    tuned_parameters <- tune.svm(Blue ~  Nb_color_cat + log_dist2lakes + I(Nb_color_cat^2), data = data_train, gamma = 10^(-5:-1), cost =10^(-3:1))
  
    # fit SVM with tuned parameters on training data
    svm_model_after_tune <- svm(Blue ~  Nb_color_cat + log_dist2lakes + I(Nb_color_cat^2), data_train, kernel="radial", cost=tuned_parameters$best.parameters[1,2], gamma=tuned_parameters$best.parameters[1,1])
...
if( !file.exists("./cached_results/mediation_dalto.RData") )
{
  ## WITH ALL DATA, with normal regression: lat --> UVB_z --> Blue 
  model_mediator <- bf(UVB_z ~ lat_rescaled + (1 | macroarea) + (1 | family_code)) + gaussian()
  model_outcome <- bf(daltonism_beta ~ UVB_z + lat_rescaled + (1 | macroarea) + (1 | family_code)) + Beta()
  dalto_lat_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)
  
  ## WITH ALL DATA, with normal regression: lat² --> UVB_z --> Blue
  # model_mediator <- bf(UVB_z ~ I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code))  + gaussian()
  # model_outcome <- bf(daltonism_beta ~ UVB_z + I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code)) + Beta()
  # dalto_lat2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
  #           cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
  #           control=brms_control)
  ...
  ## WITH ALL DATA, with normal regression: lat+lat²  --> UVB_z --> Blue 
  model_mediator <- bf(UVB_z ~ lat_rescaled + I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code))  + gaussian()
  model_outcome <- bf(daltonism_beta ~ UVB_z + lat_rescaled + I(lat_rescaled^2) + (1 | macroarea) + (1 | family_code)) + Beta()
  dalto_latlat2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)
  
  ### WITH latitude family
  
  ## WITH ALL DATA, with normal regression: latfam --> UVB_z --> Blue 
  model_mediator <- bf(UVB_z ~ lat_fam_rescaled + (1 | macroarea)) + gaussian()
  model_outcome <- bf(daltonism_beta ~ UVB_z + lat_fam_rescaled + (1 | macroarea)) + Beta()
  dalto_latfam_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)
  ...
  # ## WITH ALL DATA, with normal regression: latfam² --> UVB_z --> Blue
  # model_mediator <- bf(UVB_z ~ I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code))  + gaussian()
  # model_outcome <- bf(daltonism_beta ~ UVB_z + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code)) + Beta()
  # dalto_latfam2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
  #           cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
  #           control=brms_control)
  
  ## WITH ALL DATA, with normal regression: latfam+latfam²  --> UVB_z --> Blue 
  model_mediator <- bf(UVB_z ~ lat_fam_rescaled + I(lat_fam_rescaled^2) + (1 | macroarea))  + gaussian()
  model_outcome <- bf(daltonism_beta ~ UVB_z + lat_fam_rescaled + I(lat_fam_rescaled^2) + (1 | macroarea)) + Beta()
  dalto_latfamlatfam2_UV <- brm(model_mediator + model_outcome + set_rescor(FALSE), data = bNA,
            cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
            control=brms_control)
...
{
  # fit model, with only UVB as a fixed effect
  model_uvb_rs <- brm(daltonism_beta ~ UVB_z + (1 + UVB_z | macroarea) + (1 + UVB_z | family_code), 
                      data=bNA,
                      family="beta",
                      cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control); 
  summary(model_uvb_rs);
...
The effect of UVR on daltonism is *strong* and *negative*:
 ...
 -  Bayes factor `r sprintf("%.2f",bf_uvb_0$bf)`, `r BF_interpretation(bf_uvb_0$bf, "model with UVB incidence", "null model")`; `r brms_ci*100`% HDI for *&beta;*~UVR~ = [`r round(m_hdi$CI_low[m_hdi$Parameter == "b_UVB_z"],2)`, `r round(m_hdi$CI_high[m_hdi$Parameter == "b_UVB_z"],2)`]
...
# add hg: slightly better (2.14809)
# add longitude and hg: slightly better (2.11731) 

if( !file.exists("./cached_results/daltonism_uvb_bestmodel.RData") )
{

  # With beta distribution: best model
  d_bestmodel <- brm(daltonism_beta ~  Blue + lat_rescaled +  lat_fam_rescaled + I(lat_rescaled^2) + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code),
                     data=bNA, # very similar notation to lmer
                     family="beta",
                     cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
d_bestmodel_1 <- brm(daltonism_beta ~  hg + Blue + lat_rescaled +  lat_fam_rescaled + I(lat_rescaled^2) + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code),
                     data=bNA, # very similar notation to lmer
                     family="beta",
                     cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
# BF
cat(paste0("Bayes factor: best model with HG vs best model without HG: ",round(d_bf_bestmodel1$bf,3),"; ",BF_interpretation(d_bf_bestmodel1$bf, "best light model with HG", "best light model without HG"),".\n"));
...
{
  d_bestlightmodel <- brm(daltonism_beta ~  Blue + UVB_z + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code),
                         data=bNA,
                         family="beta",  
                         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
{
  d_bestlightmodel_1 <- brm(daltonism_beta ~  hg + Blue + UVB_z + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code),
                         data=bNA,
                         family="beta",  
                         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
# BF
cat(paste0("Bayes factor: best model with HG vs best model without HG: ",round(d_bf_bestlightmodel1$bf,3),"; ",BF_interpretation(d_bf_bestlightmodel1$bf, "best light model with HG", "best light model without HG"),".\n"));
...
if( !file.exists("./cached_results/daltonism_bestmodel_kfold_nouvb.RData") )
{
  # Best model (KFOLD) without UVB
   d_bestlightmodel_nouvb <- brm(daltonism_beta ~  Blue + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code),
                         data=bNA,
                         family="beta",  
                         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE, control=brms_control);
...
# plot with UVR
ggplot(bNA, aes(x=UVB_z, y=daltonism))+ geom_point(aes(colour=Blue, size=hg)) + 
  theme_bw() +
...
# plot UVR
bNA$dalto_range <- cut(bNA$daltonism, c(-Inf, median(bNA$daltonism),  Inf), 
                      labels=c("low", "high"))

ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y = latitude, fill=UVB, shape=dalto_range), color="red") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
# plot with latitude
ggplot(bNA, aes(x=lat_rescaled, y=daltonism))+ geom_point(aes(colour=Blue, size=hg)) + 
  theme_bw() +
  geom_smooth(colour="black") +
  labs(x="latitude") +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
...
However, one should note that only `r nrow(bNA[bNA$hg=="yes",])` hunter-gatherers groups are present in our dataset.
...
```{r plot predictor dalto hg, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Subsistence strategy on daltonism rate.")}

ggplot(bNA, aes(x=hg, y=daltonism, color=hg)) + 
  theme_bw() +
...
ggplot(bNA, aes(x=Blue, y=daltonism, color=Blue)) + 
  theme_bw() +
  geom_point(alpha=0.4, position=position_jitter(h=0.0,w=0.3)) + 
  geom_violin(alpha=0.2) +
  theme(axis.text.x = element_text(size =15, face = "bold"), 
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA, aes(x = longitude, y = latitude, fill=lat_fam_rescaled, shape=dalto_range), color="red") +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
bNA3 <- bNA
bNA3$supra_fam_new <- bNA3$supra_fam_name
bNA3$supra_fam_new <- as.character(bNA3$supra_fam_new)
for (el in as.character(unique(bNA3$supra_fam_name))){
  if (nrow(bNA3[bNA3$supra_fam_name==el,]) == 1) {
    bNA3$supra_fam_new[bNA3$supra_fam_name==el] <- "z_Isolated"
  }
}
...
ggplot() + 
  theme_bw() + 
  geom_point(data=bNA3, aes(x=lat_fam_rescaled, y=daltonism, color=supra_fam_new)) + 
  geom_smooth(data=bNA3, aes(x=lat_fam_rescaled, y=daltonism)) +
...
# subdf1 <- data.frame(ID=bNA3$supra_fam_new, daltonism = bNA3$daltonism, lat_fam = bNA3$lat_fam_rescaled)
# subdf1 <- aggregate(list(dalto=subdf1$daltonism), by=list(ID=subdf1$ID, lat=subdf1$lat_fam), FUN=max)
...
if( !file.exists("./cached_results/daltonism_var_importance.RData") )
{
  ...
  list_variables <- c("UVB_z" , "Blue"  , "hg" , "lat_rescaled", "lon_rescaled" , "log_popSize" , "climate_PC1", "climate_PC2" , "alt_rescaled"  , "log_dist2water", "log_dist2ocean" , "log_dist2lakes", "log_dist2rivers"  , "lat_fam_rescaled", "lon_fam_rescaled")
  ...
  for (it in 1:dn_iter_gini)
    ...
    # compute random forest algorithm on all variable except multicollinear
    rf.db = randomForest(daltonism ~  UVB_z + Blue  + hg + lat_rescaled + lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2water + log_dist2ocean + log_dist2lakes + log_dist2rivers  + lat_fam_rescaled + lon_fam_rescaled, data = bNA, importance=TRUE)
    
    # check the difference between actual and predicted value
    vec_original_predicted <- abs(bNA$daltonism - rf.db$predicted )
    ...
    rsquared_rf[it] <- 1-(sum((vec_original_predicted)^2)/sum((bNA$daltonism - mean(bNA$daltonism))^2))
    ...
    # use an other algorithm to compute the accuracy-based variable importance
     obj <- cforest(daltonism ~  UVB_z + Blue  + hg + lat_rescaled + lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2water  + log_dist2ocean + log_dist2lakes + log_dist2rivers  + lat_fam_rescaled + lon_fam_rescaled, data = bNA)
    
    # check the difference between actual and predicted value
    oobPredicted <- predict(obj,OOB=T)
    vec_original_predicted <- abs(bNA$daltonism - oobPredicted )
  
    # compute mse, mae, rmse, Rsquared and save those data in table
    mse_cf[it] <- mean((vec_original_predicted)^2)
    mae_cf[it] <- mean(abs(vec_original_predicted))
    rmse_cf[it] <- sqrt(mse_cf[it])
    rsquared_cf[it] <- 1-(sum((vec_original_predicted)^2)/sum((bNA$daltonism - mean(bNA$daltonism))^2))
...
ggplot(mapping = aes(x = reorder(namevar, mean_all), y = mean_all, fill=reorder(namevar, mean_all)), 
    ...
    scale_x_discrete(labels=c("hg" = "Subsistence mode", "lon_fam_rescaled" = "Long of family origin", "lat_fam_rescaled" = "Lat of family origin", "log_dist2ocean" = "Dist to ocean","log_dist2rivers" = "Dist to rivers", "lon_rescaled" = "Longitude", "alt_rescaled" = "Altitude", "log_dist2water" = "Dist to water", "climate_PC1" = "Climate PC1", "climate_PC2" = "Climate PC2", "log_dist2lakes" = "Dist to lakes", "lat_rescaled" = "Latitude", "log_popSize" = "Population size", "UVB_z" = "UVR","Blue" = "Blue", "Nb_color_cat"="Nb of colour categ")) +
      guides(fill=FALSE)
```
...
tree2 <- ctree (daltonism ~  UVB_z + Blue  + hg + lat_rescaled + lon_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, 
                data = bNA, 
                control = ctree_control(testtype = "MonteCarlo", minsplit = 6, minbucket = 6, maxdepth=30))
...
bNA_map <- bNA
# create range
bNA_map$lon_range <- cut(bNA_map$lon_rescaled, c(-Inf, nb_lon, Inf), 
                                labels=c(paste("less than", nb_lon), paste("more than", nb_lon)))

bNA_map$UVB_range2 <- cut(bNA_map$UVB_z, c(-Inf, nb_UVB, Inf), 
                                labels=c(paste("less than", nb_UVB), paste("more than", nb_UVB)))

bNA_map$UVB_range3 <- cut(bNA_map$UVB_z, c(-Inf, nb_UVB2, Inf), 
                                labels=c(paste("less than", nb_UVB2), paste("more than", nb_UVB2)))

bNA_map$docean_range <- cut(bNA_map$log_dist2ocean, c(-Inf, nb_distocean, Inf), 
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA_map, aes(x =longitude, y = latitude, color=UVB_range2, shape=dalto_range)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data= bNA_map, aes(x =longitude, y = latitude, color=UVB_range3, shape=dalto_range)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA_map, aes(x = longitude, y = latitude, color=docean_range, shape=dalto_range)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
ggplot() + theme_bw() +
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group) ,fill = "grey") +
  geom_point(data=bNA_map, aes(x = longitude, y =latitude, color=lon_range, shape=dalto_range)) +
  theme(legend.position = c(0.75, 0.5), 
      legend.justification = c(1, 1), 
      legend.title = element_text(size = 9), 
      legend.text = element_text(size = 10)) +
...
<img src="./InputFiles/formulas.jpg" alt="Formulas" style="height:in;"/>
...
if( !file.exists("./cached_results/daltonism_mach_learning.RData") )
{
  ...
  for (k in 1:30){
    ...
    # split data in training and testing set
    data_split <- initial_split(bNA, prop=0.80, strata = "daltonism_beta")
    data_train <- training(data_split)
    data_test <- testing(data_split)
    ...
    # fit randomforest on training dat
    rf.db = randomForest(daltonism_beta ~ UVB + Blue  + hg + lon_rescaled + lat_rescaled + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, data = data_train)
    
    # predict on testing set
    pred <- predict(object = rf.db, newdata = data_test)
    
    # check the difference between actual and predicted value
    vec_original_predicted <- abs(data_test$daltonism_beta - pred )
    ...
    rsquared_all[k, "randomForest"] <- 1-(sum((vec_original_predicted)^2)/sum((data_test$daltonism_beta - mean(data_test$daltonism_beta))^2))
    
    # ----- BRMS ---- #
    
    # fit brms model on training data
    brms_model <- brm(daltonism_beta ~ UVB + Blue  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled + (1 | macroarea) + (1 | family_code), 
                      family="beta",
                     data=data_train,
                     cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
                     control=brms_control); 
    ...
    # check the difference between actual and predicted value
    vec_original_predicted <- abs(data_test$daltonism_beta - pred[,1] )
    ...
    rsquared_all[k, "brms"] <-  1-(sum((vec_original_predicted)^2)/sum((data_test$daltonism_beta - mean(data_test$daltonism_beta))^2))
    
    # ------- SVM -------- #
    
    # find best parameters for SVM
    tuned_parameters <- tune.svm(daltonism_beta ~ UVB + Blue  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, data = data_train, gamma = 10^(-5:-1), cost = 10^(-3:1))
  
    # fit SVM with tuned parameters on training data
    svm_model_after_tune <- svm(daltonism_beta ~ UVB + Blue  + hg + lon_rescaled + lat_rescaled  + log_popSize + climate_PC1 + climate_PC2 + alt_rescaled  + log_dist2ocean + log_dist2lakes + log_dist2rivers + Nb_color_cat + lat_fam_rescaled + lon_fam_rescaled, data_train, kernel="radial", cost=tuned_parameters$best.parameters[1,2], gamma=tuned_parameters$best.parameters[1,1])
    ...
    rsquared_all[k, "SVM"] <- 1-(sum((vec_original_predicted)^2)/sum((data_test$daltonism_beta - mean(data_test$daltonism_beta))^2))
  }
...
The unit in this graph is linked to *daltonism* unit: as a reminder, *daltonism* varies between `r min(bNA$daltonism)` and `r max(bNA$daltonism)` (percentage of color-blind males in the population). The values for *MSE*, *RMSE* and *MAE* in a good model tend to be close to 0.
...
if( !file.exists("./cached_results/daltonism_mach_learning_simple.RData") )
{
  ...
  for (k in 1:30){
    ...
    # fit randomforest on training dat
    rf.db = randomForest(daltonism_beta ~  UVB_z + Blue  + lat_fam_rescaled , data = data_train)
    ...
    # fit brms model on training data
    brms_model <- brm(daltonism_beta ~  UVB_z  + Blue  + I(lat_fam_rescaled^2) + (1 | macroarea) + (1 | family_code), 
         data=data_train,
         family="beta",
         cores=brms_ncores, iter=brms_iter, warmup=brms_warmup, thin=brms_thin, save_all_pars=TRUE,
         control=brms_control); 
    ...
    # find best parameters for SVM
    tuned_parameters <- tune.svm(daltonism_beta ~  UVB_z + Blue  + lat_fam_rescaled , data = data_train, gamma = 10^(-5:-1), cost = 10^(-3:1))
  
    # fit SVM with tuned parameters on training data
    svm_model_after_tune <- svm(daltonism_beta ~  UVB_z + Blue  + I(lat_fam_rescaled^2) , data_train, kernel="radial", cost=tuned_parameters$best.parameters[1,2], gamma=tuned_parameters$best.parameters[1,1])
...
```{r plot mach learn dalto set of cov mae rmse, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Measures of model regression accuracy (MAE and RMSE), with the model including only UVR, Blue, latitude² of language family origins.")}
ggplot(data=df_final_long[df_final_long$test=='MAE' | df_final_long$test=='RMSE',], aes(x=test, y=percentage*100, fill=Algorithm)) +
  theme_bw() +
  geom_bar(stat="identity", position=position_dodge()) +
   theme(axis.text.x = element_text(size =13, face = "bold"), 
...
```{r plot mach learn dalto set of cov rsquared, echo=FALSE, message=FALSE, warning=FALSE, fig.cap=capFig("Measures of model regression accuracy (R-squared),with the model including only UVR, Blue, latitude² of language family origins.")}
ggplot(data=df_final_long[df_final_long$test=='RSquared',], aes(x=Algorithm, y=percentage*100, fill=Algorithm)) +
  geom_bar(stat="identity", position=position_dodge()) +
  theme_bw() +
  labs( x ="", y = "Percentage of variance") +
...
#The regression performance of *SVM* is higher than regression performance for *randomForest* and *brms* when all covariates are included. However, when only *UVR*, *subsistence mode*, *Blue*, *latitude* and *longitude* of language family are present, all algorithm give the same performance. 
...
<img src="./InputFiles/conclusion.png" alt="Results" style="height:in;"/>
